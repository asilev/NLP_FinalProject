...
public class AllFeaturesModel {
	public static List<TaggedFeatureVector<String>> buildAllFeatures(Map<String, List<List<MorphemeRecord>>> userTweets, String ZERO_INDEX) throws Exception {
		...
		for (int i=0;i<usersTweetsVectorForBagOfWords.size();++i) {
			...
			if (!tag.equals(usersTweetsVectorForBagOfWords.get(i).getTag())) {
				throw new Exception("tags are not equals");
			}
